...
const mockOneClickAppJson = JSON.stringify({
  caproverOneClickApp: {
    variables: [
      {
        id: "$$cap_db_pass",
...
describe("CaproverAPI", () => {
  ...
  it("test_list_apps: should fetch a list of apps", async () => {
    ...
    mockedAxios.get.mockResolvedValue({
      data: mockSuccessResponse(mockAppList),
    });
  ...
  it("test_create_app: should send a correct request to create an app", async () => {
    ...
    mockedAxios.get.mockResolvedValue({
      data: mockSuccessResponse({ isAppBuilding: false }),
    });
  ...
  it("test_update_app: should merge current and new app data correctly", async () => {
    // Arrange
    const appName = "app-1";
    const currentApp = mockAppList.appDefinitions.find(
      (a) => a.appName === appName,
    );
    const updates = {
      instanceCount: 5,
  ...
  it("test_deploy_app: should send the correct payload for image deployment", async () => {
    ...
    expect(mockedAxios.post).toHaveBeenCalledWith(
      `/api/v2/user/apps/appData/${appName}`,
      {
  ...
  it("test_deploy_one_click_app: should deploy services in the correct order", async () => {
    ...
    mockedAxios.get.mockImplementation((url: string) => {
      ...
      return Promise.reject(
        new Error(`Unexpected GET request to ${url} in this test.`),
      );
    });
  ...
  it("test_create_backup: should call create and download endpoints", async () => {
    ...
    mockedAxios.post.mockResolvedValue({
      data: mockSuccessResponse({ downloadToken }),
    });
    ...
    expect(mockedAxios.post).toHaveBeenCalledWith(
      "/api/v2/user/system/createbackup",
      {
    ...
    expect(mockedFs.writeFile).toHaveBeenCalledWith(
      expect.stringContaining("my-backup.tar"),
      "backup-file-content",
    );
  });
  ...
  it("test_retry_429: should retry transient 429 errors and eventually succeed", async () => {
    // Arrange
    ...
    const tooManyRequestsError = new AxiosError(
      "Too many requests",
      undefined,
      undefined,
      undefined,
    ...
    mockedAxios.get
      ...
      .mockResolvedValueOnce({ data: mockSuccessResponse(mockAppList) });
  ...
  it("test_deploy_one_click_app_json: should parse JSON definitions from one-click source", async () => {
    // Arrange
    mockedAxios.get.mockResolvedValueOnce({ data: mockOneClickAppJson });
    mockedAxios.post.mockResolvedValue({ data: mockSuccessResponse() });
  ...
  it("test_deploy_one_click_app_yaml_extension: should parse YAML definitions when explicit extension is provided", async () => {
    // Arrange
    mockedAxios.post.mockResolvedValue({ data: mockSuccessResponse() });

    mockedAxios.get.mockImplementation((url: string) => {
      if (url.endsWith("/my-app.yml")) {
        return Promise.resolve({ data: mockOneClickAppYaml });
      }
    ...
    // Act
    await api.deployOneClickApp("my-app.yml", "my-app-instance", {});
