...
const PUBLIC_ONE_CLICK_APP_PATH = "https://oneclickapps.caprover.com/v4/apps/";
...
export class CaproverAPI {
  ...
  private static readonly RETRY_429_SETTINGS: RetrySettings = {
    times: 6,
    delayMs: 15000,
  };
  private static readonly RETRY_NETWORK_SETTINGS: RetrySettings = {
    times: 3,
    delayMs: 1000,
  };
   ...
   * Retries transient errors with per-error policy.
   * - HTTP 429: CapRover operation lock, retry with longer delay.
   * - Network transport errors: retry with shorter delay.
   */
  private _getRetrySettings(error: unknown): RetrySettings | null {
    if (error instanceof AxiosError && error.response?.status === 429) {
      return CaproverAPI.RETRY_429_SETTINGS;
    }

    if (error instanceof AxiosError && !error.response) {
      return CaproverAPI.RETRY_NETWORK_SETTINGS;
    }

    return null;
  }

  private async _retry<T>(asyncFunc: () => Promise<T>): Promise<T> {
    const attemptByCategory: Record<string, number> = {};
    while (true) {
      try {
        return await asyncFunc();
      } catch (error) {
        const retrySettings = this._getRetrySettings(error);
        if (!retrySettings) {
          throw error;
        }
        ...
        const category =
          error instanceof AxiosError && error.response?.status === 429
            ? "429"
            : "network";
        const attempt = (attemptByCategory[category] || 0) + 1;
        attemptByCategory[category] = attempt;

        if (attempt >= retrySettings.times) {
          throw error;
        }

        console.error(
          `Attempt ${attempt} failed. Retrying in ${retrySettings.delayMs}ms...`,
          (error as Error).message,
        );
        await sleep(retrySettings.delayMs);
      }
    }
  }
  ...
  private _checkErrors<T>(response: CaproverResponse<T>): CaproverResponse<T> {
    ...
    if (
      ...
      status !== CaproverAPI.Status.STATUS_OK_PARTIALLY
    ) {
  ...
  private async _login(): Promise<void> {
    ...
    const response = await this.axios.post<CaproverResponse<{ token: string }>>(
      CaproverAPI.LOGIN_PATH,
      data,
    );
  ...
  public async getSystemInfo() {
    return this._retry(async () => {
      const response = await this.axios.get<
        CaproverResponse<{ rootDomain: string }>
      >(CaproverAPI.SYSTEM_INFO_PATH);
  ...
  public async listApps() {
    return this._retry(async () => {
      const response = await this.axios.get<
        CaproverResponse<{ appDefinitions: AppDefinition[] }>
      >(CaproverAPI.APP_LIST_PATH);
      return this._checkErrors(response.data);
    });
  }
  ...
  public async getApp(appName: string): Promise<AppDefinition | null> {
    ...
    return (
      appList.data.appDefinitions.find(
        (app: AppDefinition) => app.appName === appName,
      ) || null
    );
  }
  ...
  public async createApp(
    ...
    const response = await this._retry(() =>
      this.axios.post(CaproverAPI.APP_REGISTER_PATH, data),
    );
  ...
  public async addDomain(appName: string, customDomain: string) {
    ...
    const response = await this._retry(() =>
      this.axios.post<CaproverResponse<any>>(
        CaproverAPI.ADD_CUSTOM_DOMAIN_PATH,
        data,
      ),
    );
    return this._checkErrors(response.data);
  ...
  public async enableSsl(appName: string, customDomain?: string) {
    ...
    if (customDomain) {
      console.log(
        `${appName} | Enabling SSL for custom domain: ${customDomain}`,
      );
    ...
    const response = await this._retry(() =>
      this.axios.post<CaproverResponse<any>>(path, data),
    );
    return this._checkErrors(response.data);
  }
  ...
  public async updateApp(
    appName: string,
    updates: Partial<AppDefinition> & { [key: string]: any },
  ) {
    ...
    if (updates.envVars) {
      const currentEnvVars = currentAppInfo.envVars.reduce(
        (acc: any, v: any) => ({ ...acc, [v.key]: v.value }),
        {},
      );
      const newEnvVars = updates.envVars.reduce(
        (acc: any, v: any) => ({ ...acc, [v.key]: v.value }),
        {},
      );
      const merged = { ...currentEnvVars, ...newEnvVars };
      updates.envVars = Object.entries(merged).map(([key, value]) => ({
        key,
        value,
      }));
    }
    ...
    const response = await this._retry(() =>
      this.axios.post(CaproverAPI.UPDATE_APP_PATH, data),
    );
    return this._checkErrors(response.data);
  }
  ...
  public async triggerBuild(appName: string) {
    ...
    if (!pushWebhookToken) {
      throw new Error(
        `Cannot trigger build: App "${appName}" does not have a Git repository configured or push webhook enabled.`,
      );
    }
    ...
    const response = await this._retry(() =>
      this.axios.post<CaproverResponse<any>>(
        CaproverAPI.TRIGGER_BUILD_PATH,
        data,
        { params },
      ),
    );
  ...
  public async deployApp(
    appName: string,
    options: { imageName?: string; dockerfileLines?: string[] },
  ) {
    ...
    const response = await this._retry(() =>
      this.axios.post(`${CaproverAPI.APP_DATA_PATH}/${appName}`, data),
    );
  ...
  public async deleteApp(appName: string, deleteVolumes: boolean = false) {
    ...
    const response = await this._retry(() =>
      this.axios.post(CaproverAPI.APP_DELETE_PATH, data),
    );
    return this._checkErrors(response.data);
  }
  ...
  public async deployOneClickApp(
    ...
    console.log(
      `Starting one-click deployment for ${oneClickAppName} as ${appName}`,
    );

    // 1. Download app definition
    const rawAppDefinition = await this._downloadOneClickAppDefn(
      oneClickRepository,
      oneClickAppName,
    );

    // 2. Resolve variables
    const resolvedAppData = this._resolveAppVariables(
      rawAppDefinition,
      appName,
      appVariables,
    );
    const appData = this._parseOneClickDefinition(resolvedAppData);
    ...
    while (deployed.size < serviceNames.length) {
      ...
      for (const serviceName of servicesToProcess) {
        ...
        const canDeploy = dependencies.every((dep: string) =>
          deployed.has(dep),
        );
        ...
        if (canDeploy) {
          ...
          const updates: any = {
            instanceCount: 1,
            envVars: Object.entries(environment_variables).map(
              ([key, value]) => ({ key, value }),
            ),
      ...
      if (deployedInThisPass === 0 && servicesToProcess.size > 0) {
        throw new Error(
          `Circular dependency or missing dependency detected. Cannot deploy: ${[...servicesToProcess].join(", ")}`,
        );
      }
    }
  ...
  public async createBackup(fileName?: string) {
    ...
    const createResponse = await this._retry(() =>
      this.axios.post<CaproverResponse<{ downloadToken: string }>>(
    ...
    const downloadResponse = await this._retry(() =>
      this.axios.get(CaproverAPI.DOWNLOAD_BACKUP_PATH, {
  ...
  private async getAppInfo(appName: string) {
    return this._retry(async () => {
      const response = await this.axios.get<CaproverResponse<AppData>>(
        `${CaproverAPI.APP_DATA_PATH}/${appName}`,
      );
      return this._checkErrors(response.data);
    });
  }
  ...
  private async _downloadOneClickAppDefn(
    repositoryPath: string,
    oneClickAppName: string,
  ): Promise<string> {
    const baseRepositoryPath = repositoryPath.endsWith("/")
      ? repositoryPath
      : `${repositoryPath}/`;
    const hasExtension = /\.(json|ya?ml)$/i.test(oneClickAppName);
    const candidates = hasExtension
      ? [oneClickAppName]
      : [
          `${oneClickAppName}.json`,
          `${oneClickAppName}.yml`,
          `${oneClickAppName}.yaml`,
        ];

    let lastError: unknown;

    for (const candidate of candidates) {
      const url = `${baseRepositoryPath}${candidate}`;
    ...
    throw (
      lastError ??
      new Error(
        `Could not fetch one-click app definition for ${oneClickAppName}`,
      )
    );
  }

  private _parseOneClickDefinition(rawContent: string): any {
    try {
      return JSON.parse(rawContent);
    } catch {
      return yaml.load(rawContent) as any;
    }
  }
  ...
  private _resolveAppVariables(
    ...
    rawAppData = rawAppData.replace(
      /\$\$cap_gen_random_hex\((\d+)\)/g,
      (_, lengthStr) => {
    ...
    const appDefn = this._parseOneClickDefinition(rawAppData);
    const requiredVars: AppVariable[] =
      appDefn?.caproverOneClickApp?.variables || [];

    for (const reqVar of requiredVars) {
      // FIX: These checks are now valid due to the Record<string, ...> type.
      if (
        allVariables[reqVar.id] === undefined ||
        allVariables[reqVar.id] === null
      ) {
        if (reqVar.defaultValue !== undefined) {
          allVariables[reqVar.id] = reqVar.defaultValue;
        } else {
          throw new Error(
            `Missing required variable: ${reqVar.label} (${reqVar.id}). Description: ${reqVar.description}`,
          );
        }
      }
    }
